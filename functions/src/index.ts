 ...
 */
export const parseListItem = functions.https.onCall(
  async (data: ParseItemRequest, context: functions.https.CallableContext): Promise<ParsedItemResult> => {
...
Return ONLY raw JSON, no markdown wrapper.`;
        ...
        if (jsonMatch) {
          const parsed = JSON.parse(jsonMatch[0]) as ParsedItemResult;
          return {
 ...
 */
function parseRuleBased(text: string): ParsedItemResult {
  ...
  if (lower.includes('kilo') || lower.includes('kg')) unit = 'kg';
  if (lower.includes('liter') || lower.includes('litre') || lower.includes('ltr')) unit = 'liter';
  if (lower.includes('dozen') || lower.includes('dazan') || lower.includes('dazzen')) unit = 'dozen';
...
function normalizeUnit(u: string): string {
  ...
  if (['kg', 'kilo', 'kilogram', 'kilos'].includes(lower)) return 'kg';
  if (['g', 'gram', 'grams'].includes(lower)) return 'gram';
  if (['l', 'liter', 'litre', 'liters'].includes(lower)) return 'liter';
  if (['pkt', 'packet', 'packets'].includes(lower)) return 'packet';
  if (['doz', 'dozen', 'dazan'].includes(lower)) return 'dozen';
  return u;
}

function categorizeFallback(name: string): ParsedItemResult['category'] {
 ...
 */
export const onItemBought = functions.firestore
  .document('households/{householdId}/listItems/{itemId}')
